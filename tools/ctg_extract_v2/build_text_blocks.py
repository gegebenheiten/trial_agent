...
from extract.text_blocks import extract_text_blocks


def parse_args() -> argparse.Namespace:
    project_root = Path(__file__).resolve().parents[2]
    parser = argparse.ArgumentParser(description="Build CTG text blocks (v2) from CT.gov XML.")
    parser.add_argument(
        "--xml-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Root output directory for CTG text blocks (<NCTID>/ctg_text_blocks.jsonl).",
    )
    parser.add_argument(
        "--nct-id",
        "--nct_id",
...
def main() -> None:
    ...
    if nct_ids:
        if len(nct_ids) == 1:
            nct_id = nct_ids[0]
            per_nct_text = args.output_root / nct_id / "ctg_text_blocks.jsonl"
